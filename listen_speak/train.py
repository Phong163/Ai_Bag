from listen_speak.model import build_transformer
from listen_speak.dataset import BilingualDataset, causal_mask
#Import class BilingualDataset và hàm causal_mask từ file dataset.py để tạo và xử lý tập dữ liệu.
...
import warnings #Import module warnings - thư viện cho phép hiển thị cảnh báo trong quá trình chạy chương trình.
from tqdm import tqdm #thư viện hỗ trợ hiển thị tiến trình trong vòng lặp.
import os #thư viện cho phép thao tác với các đường dẫn và hệ thống tệp tin.
from pathlib import Path #thư viện cho phép làm việc với đường dẫn và đường dẫn tệp tin.
...
def get_or_build_tokenizer(config, ds, question, answer):
    ...
    if not Path.exists(tokenizer_path_question) and  not Path.exists(tokenizer_path_answer):
...
def get_ds(config):
    ...
    ds_r = []
    ds_raw = []
    with open(r'C:\Users\OS\Desktop\AiBag\listen_speak\output.jsonl', "r", encoding="utf-8") as file:
        for line in file:
          try:
    ...
    for item in ds_r:
        ds_raw_item = item["dialog"]
        ds_raw.extend(ds_raw_item)#ds_raw[{'question':'','answer':''},{'question':'','answer':''},..]
    ...
    for item in ds_raw:
      ...
      # Tiếp tục xử lý src_ids và tgt_ids tại đây nếu item đủ cả 'question' và 'answer'
